...
from perfora.policy import PolicyError, RepositoryPolicy, load_policy
...
def test_policy_requires_suppression_reason_and_expiry(tmp_path: Path) -> None:
    fingerprint = assign_fingerprints([_finding(file="lib/a.dart", symbol="api")])[0][
        "fingerprint"
    ]
    policy_path = tmp_path / ".perfora.yaml"
    policy_path.write_text(
...
def test_report_classifies_baseline_and_gates_only_new_findings(tmp_path: Path) -> None:
    ...
    assert all(not item["locations"][0]["physicalLocation"]["artifactLocation"]["uri"].startswith("/") for item in results)
...
def test_repository_suppression_removes_finding_from_gate(tmp_path: Path) -> None:
    ...
    fingerprint = assign_fingerprints([{**finding, "audit_type": "security"}])[0][
        "fingerprint"
    ]
...
def test_archives_baseline_without_mutating_current_worktree(tmp_path: Path) -> None:
    ...
    subprocess.run(["git", "config", "user.email", "test@perfora.local"], cwd=repository, check=True)
...
def test_cli_maps_analyzer_failures_to_stable_exit_codes(
    ...
    result = cli.run_cli(
        ["audit", "--repository", str(tmp_path), "--type", "security"]
    )
